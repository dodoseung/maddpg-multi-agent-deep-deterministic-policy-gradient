...
class DDPG():
    def __init__(self, env, n_agents, memory_size=10000000, batch_size=64, tau=0.01, gamma=0.95, learning_rate=1e-3, eps_min=0.05, eps_period=10000, bn=False):
        ...
        # Actors
        self.actors_net = [ActorNet(self.state_num, self.action_num, self.action_min, self.action_max, bn).to(self.device) for _ in range(n_agents)]
        self.actors_opt = [optim.Adam(n.parameters(), lr=learning_rate) for n in self.actors_net]
        
        # Target Actors
        self.actors_target_net = [ActorNet(self.state_num, self.action_num, self.action_min, self.action_max, bn).to(self.device) for _ in range(n_agents)]
        self.actors_target_net.load_state_dict(self.actors_net.state_dict())
        
        # Critics
        self.critics_net = [CriticNet(self.state_num, self.action_num, bn).to(self.device) for _ in range(n_agents)]
        self.critics_opt = [optim.Adam(n.parameters(), lr=learning_rate) for n in self.critics_net]
        
        # Target Critics
        self.critics_target_net = [CriticNet(self.state_num, self.action_num, bn).to(self.device) for _ in range(n_agents)]
